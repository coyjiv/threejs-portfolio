import { Project, projects as projectsData } from '@/constants/projects'
...
const ProjectsPage = () => {
  return (
    <section className='p-10 flex flex-col items-center '>
      <h1 className='text-3xl sm:text-5xl font-bold font-ds text-shadow'>Projects</h1>
  ...
  function ProjectItem({name, description, category, github, image, link, tags}: Project): React.ReactElement {

    const [squashed, setSqashed] = useState(true)
    ...
    >
      <div className='flex flex-col gap-3 max-w-xl px-4 py-3'>
        ...
        <p className={`text-sm h-auto max-transition ${squashed ? "sm:line-clamp-3 max-h-[200px]": "max-h-[9999px]"} text-black `}>
          {description}
        </p>
          <button className={`text-left text-sm font-semibold text-zinc-600 ${description.length<250? 'hidden': ''}`} onClick={()=>setSqashed(!squashed)}>Click to {!squashed?"view less":"view full"}</button>
